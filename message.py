class Message:
    def __init__(self, msg_type, sender_id, id_negotiation, price, state="processing", message_number=0, message_remaining=10):
        """
    ...
    def __str__(self):
        ...
        if str(self.state) == "accepted":
                return f"------ END OF NEGOCIATION : {self.id_negotiation} - Sold for ${self.price}  ------"
        ...
        return f"{self.type}.{self.id} offers to sell for ${self.price} ({self.message_remaining} messages left, state:{self.state})"
